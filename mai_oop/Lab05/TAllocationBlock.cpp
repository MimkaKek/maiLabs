...
TAllocationBlock::TAllocationBlock(size_t size,size_t count): _size(size),_count(count)  {
    ...
    for(size_t i = 0; i < _count; ++i) {
        _queue_free_blocks.push(std::shared_ptr<char>(_used_blocks + (i*_size)));
    }
    _free_count = _count;
...
void* TAllocationBlock::allocate() {
    ...
    if(_free_count > 0) {
        result = (_queue_free_blocks.pop()).get();
        --_free_count;
...
void TAllocationBlock::deallocate(void* pointer) {
    ...
    _queue_free_blocks.push(std::shared_ptr<char>((char*)pointer));
    _free_count++;
