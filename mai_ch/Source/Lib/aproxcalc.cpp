...
TMatrix NewtonMethod(TMatrix& X0, double eps) {
    ...
    for(size_t iter = 0; DX.CalcNorm() > eps; ++iter) {
        ...
        if(ans.first.GetDeterminant() < eps) {
            std::cout << "Determinant of Jacobi matrix is too low! Return X = (0, 0)" << std::endl;
            return TMatrix(2, 1);
        }
        DX = SolveSystemViaLU(ans.first, ans.second);
        X += DX;
...
TMatrix SimpleIterations(TMatrix& X0, TMatrix A, TMatrix B, double eps) {
    ...
    if(mnorm >= 1) {
        std::cout << "Conditions are not met! Return X = (0, 0)" << std::endl;
        return TMatrix(2, 1);
    }
    ...
    DX = NX - X;
    for(size_t iter = 0; DFi.CalcNorm() > eps; ++iter) {
        X = NX;
        NX[0][0] = SFi1(X[0][0], X[1][0]);
        NX[1][0] = SFi2(X[0][0], X[1][0]);
        DX = NX - X;
