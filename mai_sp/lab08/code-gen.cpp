...
int tCG::p01(){ // S -> PRED
  ...
  S1->obj = header + S1->obj;
	return 0;}
int tCG::p02(){ //     PRED -> HPRED BOOL )
 S1->obj += S2->obj+";\n}\n";
	return 0;}
int tCG::p03(){ //    HPRED -> PDPAR )
 S1->obj += ")";
 declarations += S1->obj + ";\n"; //!!!
 S1->obj += "{\n  return ";
 S1->count = 0;
	return 0;}
int tCG::p04(){ //    PDPAR -> ( define ( $idq
 S1->obj = "bool " + decor(S4->name) + "(";
 S1->count = 0;
	return 0;}
int tCG::p05(){ //    PDPAR -> PDPAR $idq

	if(S1->count)S1->obj += ", ";
	S1->obj = "bool " + decor(S2->name);
	++(S1->count);
	return 0;}
int tCG::p06(){ //    PDPAR -> PDPAR $id
 if(S1->count)S1->obj += ", ";// не первый параметр
 S1->obj += "double " + decor(S2->name);
 ++(S1->count);
	return 0;}
int tCG::p07(){ //     BOOL -> $bool
 S1->obj = (S1->name=="#t" ? "true" : "false");
	return 0;}
int tCG::p08(){ //     BOOL -> $idq
 S1->obj = decor(S1->name);
	return 0;}
int tCG::p09(){ //     BOOL -> ( = E E )
	S1->obj = S3->obj + "==" + S4->obj;
	return 0;}
int tCG::p10(){ //     BOOL -> ( not BOOL )

	S1->obj += "!" + S2->obj;
	return 0;}
int tCG::p11(){ //     BOOL -> ( or BOOL BOOL )
